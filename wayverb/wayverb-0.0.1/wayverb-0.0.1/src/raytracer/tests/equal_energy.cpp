    ...
    const auto histogram_energy = [&] {
        const auto histogram =
                compute_summed_histogram(std::get<0>(*results), attenuator)
                        .histogram;
        const float dist = glm::distance(source, receiver);
        const float sr = static_cast<float>(params.histogram_sample_rate);
        const float sos = static_cast<float>(environment.speed_of_sound);
        const size_t histogram_bin = static_cast<size_t>(dist * sr / sos);
