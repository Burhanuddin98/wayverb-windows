...
auto run(
        ...
        Callbacks&& callbacks) {
    fprintf(stderr, "[rt] allocating scene_buffers on GPU\n"); fflush(stderr);
    const core::scene_buffers buffers{cc.context, voxelised};
    fprintf(stderr, "[rt] scene_buffers OK\n"); fflush(stderr);
    ...
    fprintf(stderr, "[rt] constructing processors\n"); fflush(stderr);
    ...
    fprintf(stderr, "[rt] processors OK\n"); fflush(stderr);
    ...
    fprintf(stderr, "[rt] reflection_depth=%zu segment_size=%d\n",
            reflection_depth, segment_size); fflush(stderr);

    const auto run_segment = [&](auto b, auto e) {
        const auto num_directions = std::distance(b, e);
        fprintf(stderr, "[rt] run_segment: constructing reflector num_dirs=%zu\n",
                (size_t)num_directions); fflush(stderr);

        reflector ref{cc, receiver, make_ray_iterator(b), make_ray_iterator(e)};
        fprintf(stderr, "[rt] run_segment: reflector OK\n"); fflush(stderr);

        auto group_processors = util::apply_each(
                util::map(make_get_group_processor_functor_adapter{},
                          processors),
                std::make_tuple(num_directions));
        fprintf(stderr, "[rt] run_segment: group_processors OK\n"); fflush(stderr);
