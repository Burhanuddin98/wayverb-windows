...
auto interval_size(Engine& engine, double mean_occurrence) {
    //  Standard inverse-CDF for the exponential distribution: -ln(U)/lambda
    //  where U ~ Uniform(0,1).  Guard against the vanishingly rare U==0 case
    //  (which would produce log(0)=-inf) by clamping to the smallest positive
    //  double.  The original code used {1.0, 0.0} which is UB (a > b);
    //  GCC 15 now asserts on it.
    std::uniform_real_distribution<double> distribution{0.0, 1.0};
    const auto u = std::max(distribution(engine),
                            std::numeric_limits<double>::min());
    return -std::log(u) / mean_occurrence;
}
...
auto sum_directional_histogram(
    ...
    for (auto azimuth_index = 0ul; azimuth_index != Az; ++azimuth_index) {
        for (auto elevation_index = 0ul; elevation_index != El;
            ...
            for (size_t i = 0, end = segment.size(); i != end; ++i) {
                ret[i] += segment[i];
            }
        }
    }
