...
public:
    ...
    void do_draw(const glm::mat4& model_matrix) const {
        ...
        ++frame_count;
        if (frame_count == 1 || frame_count == 60 || frame_count == 300) {
            GLint vp[4] = {};
            glGetIntegerv(GL_VIEWPORT, vp);
            fprintf(stderr, "[view] frame=%d viewport=%d,%d,%d,%d model=%s\n",
                    frame_count, vp[0], vp[1], vp[2], vp[3],
                    model_object_ ? "YES" : "NO");
            fprintf(stderr, "[view] eye_dist=%.2f origin=%.2f,%.2f,%.2f\n",
                    current_view_.get_eye_distance(),
                    current_view_.get_origin().x,
                    current_view_.get_origin().y,
                    current_view_.get_origin().z);
            fflush(stderr);
        }
        ...
        while ((err = glGetError()) != GL_NO_ERROR) {
            fprintf(stderr, "[view] GL error: 0x%x\n", err);
            fflush(stderr);
        }
    }
